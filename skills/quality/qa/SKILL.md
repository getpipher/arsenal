...
| `--global` | (off) | Save report to `~/Documents/secret/qa-reports/<project>/`. Implies `--save`. |
...
| `--global` OR `default_save_tier: global` | Save to `~/Documents/secret/qa-reports/<project>/<run-id>/` |
...
4. Print to user: *"Reports save to `.qa/` (gitignored). Use `--commit` to publish to `docs/qa/`, or `--global` for encrypted iCloud storage."*
...
2. Parse owner from any of these URL shapes:
   - SSH: `git@<host>:<owner>/<repo>(.git)?` (e.g., `git@github.com:RECTOR-LABS/kami.git` → `RECTOR-LABS`)
   - HTTPS: `https://<host>/<owner>/<repo>(.git)?` (e.g., `https://github.com/RECTOR-LABS/kami` → `RECTOR-LABS`)
   - SSH alt: `ssh://git@<host>/<owner>/<repo>(.git)?`
   - Hosts include: `github.com`, `gitlab.com`, custom GitLab/Gitea hosts
   - If multiple remotes exist (origin, upstream, gitlab), use `origin` only. Per CLAUDE.md GitLab mirror convention, RECTOR's repos have both GitHub origin AND GitLab mirror — guard against the GitHub origin to be safe.
...
- Global: `~/Documents/secret/qa-reports/<project>/HISTORY.md`

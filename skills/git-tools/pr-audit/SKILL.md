...
/git:pr-audit --org sip-protocol  # Audit specific organization
...
check_cross_repo_dependencies() {
    ...
    if echo "$changed_files" | grep -q "packages/types/"; then
        shared_packages="$shared_packages @sip-protocol/types"
    fi
    if echo "$changed_files" | grep -q "packages/sdk/"; then
        shared_packages="$shared_packages @sip-protocol/sdk"
    fi
...
if [[ "$DRY_RUN" == true ]]; then
    ...
    # Merge with merge commit (preserve history, per global CLAUDE.md)
